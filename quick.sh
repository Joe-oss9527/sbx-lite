#!/usr/bin/env bash
# sbx-lite one-liner installer (remote-friendly)
# Usage:
#   bash <(curl -fsSL https://raw.githubusercontent.com/YYvanYang/sbx-lite/main/quick.sh)
#   bash <(wget -qO- https://raw.githubusercontent.com/YYvanYang/sbx-lite/main/quick.sh)
set -euo pipefail

# --- privilege ---
if [ "${EUID:-$(id -u)}" -ne 0 ]; then
  exec sudo -E bash "$0" "$@"
fi

REPO="${SBX_REPO:-YYvanYang/sbx-lite}"
BRANCH="${SBX_BRANCH:-main}"
INSTALL_DIR="/opt/sbx"

log(){ echo -e "\033[1;32m[*]\033[0m $*"; }
warn(){ echo -e "\033[1;33m[!]\033[0m $*"; }
err(){ echo -e "\033[1;31m[×]\033[0m $*"; exit 1; }

# --- dependencies ---
if ! command -v curl >/dev/null 2>&1 && ! command -v wget >/dev/null 2>&1; then
  log "installing curl (no curl/wget found)"
  if command -v apt-get >/dev/null 2>&1; then
    apt-get update && apt-get install -y curl
  elif command -v dnf >/dev/null 2>&1; then
    dnf install -y curl
  elif command -v yum >/dev/null 2>&1; then
    yum install -y curl
  else
    err "no package manager found to install curl"
  fi
fi

fetch(){
  # $1 url, $2 out
  if command -v curl >/dev/null 2>&1; then
    curl -fsSL "$1" -o "$2"
  else
    wget -qO "$2" "$1"
  fi
}

# --- fetch source ---
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT

if command -v git >/dev/null 2>&1; then
  log "cloning https://github.com/${REPO}.git (${BRANCH})"
  git clone --depth=1 -b "$BRANCH" "https://github.com/${REPO}.git" "$TMP/src" >/dev/null 2>&1 || err "git clone failed"
else
  TARBALL="https://codeload.github.com/${REPO}/tar.gz/${BRANCH}"
  log "downloading tarball ${TARBALL}"
  fetch "$TARBALL" "$TMP/src.tgz"
  mkdir -p "$TMP/src"
  tar -xzf "$TMP/src.tgz" -C "$TMP/src" --strip-components=1
fi

# --- run installer from repo ---
if [ ! -x "$TMP/src/scripts/install.sh" ]; then
  err "installer not found in repo (scripts/install.sh)"
fi

log "running installer"
bash "$TMP/src/scripts/install.sh"

log "done."
log "panel (via SSH tunnel): ssh -N -L 7789:127.0.0.1:7789 root@your-server"
log "then open http://127.0.0.1:7789/ to see admin credentials"
